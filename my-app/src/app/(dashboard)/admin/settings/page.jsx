...
function Avatar({ name, size = 72 }) {
  ...
  return (
    ...
    >
      <span
        style={{ fontSize: size * 0.36, fontWeight: 700, color: "#143888" }}
      >
        {initials}
      </span>
    </div>
...
function PwdField({ label, value, onChange, show, onToggle, disabled }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <label
        style={{ fontSize: 13, fontWeight: 500, color: "rgba(0,0,0,0.7)" }}
      >
        {label}
      </label>
      <div
...
function StatItem({ label, value }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      <span style={{ fontSize: 15, fontWeight: 600, color: "#143888" }}>
        {value ?? "—"}
      </span>
    </div>
  );
...
function SectionHeader({ title, subtitle, action }) {
  return (
    ...
    >
      <div>
        <h3
          style={{ fontSize: 17, fontWeight: 600, color: "#143888", margin: 0 }}
        >
          {title}
        </h3>
        {subtitle && (
...
function ProfileTab({ profile }) {
  ...
  const isAdmin = (user?.role || profile?.role) === "admin";
  ...
  function buildForm(src) {
    const s = src || profile || user || {};
    return {
      first_name: s.first_name || "",
      last_name: s.last_name || "",
      phone: s.phone || "",
      department: s.department || "",
      admin_level: s.admin_level || "regular",
      can_import_data: s.can_import_data ?? false,
      can_export_data: s.can_export_data ?? false,
      can_manage_users: s.can_manage_users ?? false,
      can_manage_system_config: s.can_manage_system_config ?? false,
      can_view_audit_logs: s.can_view_audit_logs ?? false,
    };
  }
  ...
  async function handleSave() {
    ...
    try {
      const id =
        profile?.account_id || profile?.id || user?.account_id || user?.id;
      ...
      if (isAdmin) {
        Object.assign(payload, {
          ...
          can_import_data: form.can_import_data,
          can_export_data: form.can_export_data,
          can_manage_users: form.can_manage_users,
          can_manage_system_config: form.can_manage_system_config,
          can_view_audit_logs: form.can_view_audit_logs,
        });
      }
  ...
  async function handlePwdSubmit(e) {
    ...
    if (pwdForm.current === pwdForm.newPwd) {
      setPwdErr("New password must differ from old.");
      return;
    }
    setPwdLoading(true);
    try {
              ...
              {isAdmin && (
                <>
                  ...
                  <SelectField
                    ...
                    onChange={(v) => setForm((f) => ({ ...f, admin_level: v }))}
                    options={[
...
const ABSENCE_LIMIT_OPTIONS = [5, 6, 7, 8, 10, 12, 15].map((n) => ({
  value: String(n),
  label: `${n} absences`,
}));
const WARNING_OPTIONS = [3, 4, 5, 6, 7, 8].map((n) => ({
  value: String(n),
  label: `${n} absences`,
}));
const DEADLINE_OPTIONS = [1, 2, 3, 5, 7, 10, 14].map((n) => ({
  value: String(n),
  label: `${n} day${n > 1 ? "s" : ""}`,
}));
      ...
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
        ...
        >
          <div>
            ...
            >
              Define thresholds and deadlines
            </p>
          </div>
          ...
          <SelectField
            ...
            onChange={(v) =>
              setRules((r) => ({ ...r, justificationDeadline: v }))
            }
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#4a6bc0" }}>
              Students are warned at <strong>{rules.warningThreshold}</strong>{" "}
              absences and excluded at <strong>{rules.maxAbsences}</strong>.
              Justifications must be submitted within{" "}
              <strong>
                {rules.justificationDeadline} day
                {Number(rules.justificationDeadline) > 1 ? "s" : ""}
              </strong>
              .
            </p>
          </div>
